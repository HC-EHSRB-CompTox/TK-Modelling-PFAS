...
p
ggsave(filename = "../plots/pfos_1comp.png", plot = p, width = 5, height = 3, dpi = 300,bg = "white")



# Print summary
agreement <- left_join(obs_summary, pred_summary, by = "Dose_Time") %>%
  mutate(Fold_Diff = Mean_Pred / Mean_Obs)
print(agreement, digits = 4)

mean_fold <- agreement %>%
  filter(!Dose_Time %in% c("0 (28)", "0 (56)")) %>%
  summarise(Mean_Fold_Diff = mean(Fold_Diff))
mean_fold
...
geom_ribbon(
  ...
  theme(
    ...
    panel.grid.minor = element_blank())
p
ggsave(filename = "../plots/pfoa_1comp.png", plot = p, width = 5, height = 3, dpi = 300,bg = "white")
...
pars_m$Caco2.Pab <- 1.6
pars_m$Clmetabolismc <- 0.0002402
...
pars_f$Caco2.Pab <- 1.6
pars_f$Clmetabolismc <- 0.0003341
...
#imported as ng/ml  arithmetric mean changed to geometric 
